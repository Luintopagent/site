/* script_json_fixed.js
   building.json -> DOM + навигация (Дейкстра по длинам) + показ графа + визуальный редактор (узлы/рёбра/кабинеты)
   FIXES:
   - графовый SVG больше НЕ перехватывает клики по кабинетам (pointer-events:none на корне, интерактивны только hit-элементы)
   - кабинеты можно выбирать/таскать/редактировать
   - рёбра можно "двигать" (перецеплять): потяните ребро за конец к другому узлу
*/
// --- UI helper: checkbox (global) ---
...
// State
let floors = [];              // floor divs (.floor-rooms) ordered by floor.index
...
let activeMode = null;        // null | 'room-to-room' | 'evacuate'
let routeSteps = [];          // [{ floorIndex, nodeIds: [..] }]
...
// Indexes
let roomIndex = new Map();    // roomId -> { floorIndex, element, data }
let nodeIndex = new Map();    // `${floorIndex}:${nodeId}` -> element
let globalGraph = {};         // nodeKey -> [nodeKey,..]

// Graph overlay + editor
let showGraphOverlay = false;

// Active editor: null | 'graph' | 'rooms' | 'floor' | 'stairs' (mutually exclusive)
let activeEditor = null;
let editMode = false; // kept for legacy; derived from activeEditor !== null
...
// graph edit
let graphAction = 'select';   // 'select' | 'add-node' | 'add-edge' | 'delete'
let graphConnectDragMode = false; // drag from node to node to create edge
let selectedNodeId = null;
let edgeStartNodeId = null;   // for click-connect
let draggingNodeId = null;
let dragPointerId = null;
let tempEdgeLine = null;

// edge drag: move whole edge (translate both end nodes together)
let draggingEdge = null; // {floorIndex,a,b,startPt:{x,y},startA:{x,y},startB:{x,y}}

// room edit
let roomEditEnabled = true;
let roomAction = 'select';    // 'select' | 'add-room' | 'delete'
...
// Grid snapping (invisible small grid to align nodes/rooms)
...
function createRoomButton(floorIndex, r) {
  ...
  roomBtn.addEventListener('click', (e) => {
    // In edit mode, click is used for selecting; block popup.
    if (isRoomsEditor()) {
      e.preventDefault();
      e.stopPropagation();
      return;
...
function renderBuilding() {
  ...
  building.floors
    ...
    .forEach(floor => {
      ...
      // Graph overlay (always pointer-events:none so rooms remain clickable)
      ...
      // Click on background for add-node / add-room
      floorDiv.addEventListener('pointerdown', (e) => {
        if (!isAnyEditor()) return;

        const t = e.target;

        // ignore clicks on UI panel or toggle
...
function getLocalPointFromClient(clientX, clientY, floorIndex) {
  ...
  // floorDiv is transformed by translate+scale, so convert to local:
...
// ---------- Pathfinding (Dijkstra with real lengths) ----------
...
function onGraphSvgPointerMove(e) {
  ...
  // temp edge (new edge OR edge retarget)
...
function drawGraphOverlayOnFloor(floorIndex) {
  ...
  svg.innerHTML = '';
  if (_savedTempEdge) {
    // Keep dashed preview on top while dragging a new edge
  ...
  Object.keys(graph).forEach(a => {
    (graph[a] || []).forEach(b => {
      ...
      // fat hit line (transparent) - interactive only in editMode
      ...
      hit.setAttribute('stroke-width', '24'); // bigger hit target
      ...
      hit.addEventListener('pointerdown', (e) => {
        ...
        // drag edge (select mode): move the whole edge in space (both end nodes follow cursor)
  ...
  qsa('.path-node', floorDiv).forEach(nodeEl => {
    ...
    hit.addEventListener('pointerdown', (e) => {
      ...
      // stairs editor: assign node to connector mapping (one-shot)
      ...
      // Drag-connect mode (checkbox): start dashed edge from this node, then release on another node to connect
...
function addNodeAtPoint(floorIndex, x, y) {
  ...
  selectNode(id);
  // one-shot: exit add-node mode after placing
...
function addRoomAtPoint(floorIndex, x, y) {
  ...
  selectRoom(id);
  // one-shot: exit add-room mode after placing
...
function showFloor(floorNumber) {
  ...
  // if selected room belongs to other floor, clear selection
  if (selectedRoomId) {
...
// ---------- Graph toggle + editor UI (bar + draggable panel) ----------
function setActiveEditor(next) {
  if (next === activeEditor) next = null;

  // if enabling editor, ensure graph visible
...
function ensureEditorPanelUI() {
  ...
  header.addEventListener('pointerdown', (e) => {
    ...
    // switch from translate to absolute coords on first drag
...
function renderGraphEditor(body) {
  ...
  // Connect-drag mode (drag from node to node to create edge)
...
function shiftConnectorKeysOnDelete(deleteIndex) {
  (building.connectors || []).forEach(conn => {
    ...
    Object.keys(nb).forEach(k => {
      const fi = parseInt(k, 10);
      if (!Number.isFinite(fi)) return;
      if (fi === deleteIndex) return; // remove mapping for deleted floor
...
function addNewFloorAfterCurrent() {
  ...
  // reset navigation state (safer after structural change)
  finishRoute();
  renderBuilding();
  resetNodeCenterCache();
  buildGlobalGraph();
...
// ---------- Offline QR generation (no network) ----------
...
const QRGen = (() => {
  ...
  function makeGeneratorPoly(ecLen) {
    ...
    const gen = poly.slice(1); // omit leading 1
  ...
  function bchRemainder(value, poly) {
    // Degree-based division (matches qrcode.util.BCH_* logic)
  ...
  function formatBits(mask) {
    // ECC=M => 0 (same as python-qrcode constants)
    const data = (0 << 3) | mask; // 5 bits
  ...
  function encodeText(text) {
    ...
    // Pick minimal version that fits (fast check)
    let version = 0;
    for (let v = 1; v <= 40; v++) {
...
function ensureQrUI() {
  // Button (left of evacuate-button)
  ...
  if (!_qrBtn && evacuateButton && evacuateButton.parentElement) {
    ...
    // Style to match existing floating buttons
    _qrBtn.style.position = 'fixed';
    _qrBtn.style.top = '20px';
    _qrBtn.style.right = '140px'; // left of evacuate (which is at 80px)
    ...
    // Insert before evacuateButton in DOM
...
function openQrPopup() {
  ...
  const url = window.location.href; // includes all GET params
  const encoded = encodeURIComponent(url);

  // Uses a public QR image generator service
